...
-- ── v1.1 accuracy-pass UPDATEs (intro-to-llms) ───────────────────
-- These update the DB default notes to match the v1.1 code deck.
-- Re-run: wrangler d1 execute lectures-db --remote --file=worker/seed.sql

UPDATE slides SET notes = 'Everything in an LLM is measured in tokens. A token is roughly three quarters of a word, or about four characters in English. The word unhappiness is exactly 4 tokens in GPT-4o tokenizer: un / hap / pi / ness. A leading space is part of the next word token — the string world is one token with the space. One plain emoji is 1 token; a skin-tone modifier adds 2 more tokens on top. All of this uses OpenAI o200k_base tokenizer — Claude and Gemini use different tokenizers so token counts vary across providers. Verify any string at platform.openai.com/tokenizer.', updated_at = datetime('now') WHERE id = 'tokens' AND deck_slug = 'introduction-to-llms';

UPDATE slides SET notes = 'Five benchmarks you will see in every AI launch announcement. MMLU-Pro was created by TIGER-Lab in 2024 as a harder successor to MMLU, which most frontier models now saturate. GPQA Diamond was created by Rein et al. at NYU, Cohere, and Anthropic — the questions are deliberately Google-proof. SWE-bench was created at Princeton and the University of Chicago; the Verified subset was curated by OpenAI. AIME is a human math competition run by the Mathematical Association of America — it is now saturated at 100% for frontier models. ARC-AGI was created by Francois Chollet in 2019; ARC-AGI-2 was published by the ARC Prize Foundation in 2025 and is still genuinely difficult.', updated_at = datetime('now') WHERE id = 'bench-what' AND deck_slug = 'introduction-to-llms';
